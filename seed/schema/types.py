...
class Query(object):
    ...
    processCount = graphene.Field(
        ProcessCount, query=graphene.String())
    process = graphene.Field(
        Process, id=graphene.Int(required=True))
    users = graphene.List(
    ...
    def resolve_processPagination(self, info, **kwargs):
        return resolve_pagination(
            ...
            ProcessPagination, info, **kwargs)

    # pylint: disable=C0103
    def resolve_processCount(self, info, **kwargs):
        return resolve_count(
          ProcessModel, ProcessCount, info, **kwargs)
